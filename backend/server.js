...
const jwt = require('jsonwebtoken'); // Librería para manejar autenticación JWT
const bcrypt = require('bcryptjs'); // Librería para cifrar contraseñas
const User = require('./models/catalog.model'); // Modelo de usuario
const catalogRoutes = require('./routes/catalog.routes'); // Rutas del catálogo
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    const hashedPassword = bcrypt.hashSync(password, 10); // Se utiliza bcrypt para cifrar la contraseña
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      ...
      process.env.JWT_SECRET,  // Se usa JWT_SECRET desde el archivo .env
